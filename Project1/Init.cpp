...
HINSTANCE Init::AppInst()const
{
	return mhAppInst;
}

HWND Init::MainWnd()const
{
	return mhMainWnd;
}

float Init::AspectRatio()const
{
...
void Init::CreateRtvAndDsvDescriptorHeaps()
{
	...
	if (FAILED(md3dDevice->CreateDescriptorHeap(
		&cbvHeapDesc, IID_PPV_ARGS(&mdDescriptorHeap))));
}
...
void Init::OnResize()
{
	...
	CD3DX12_RESOURCE_BARRIER commonState = CD3DX12_RESOURCE_BARRIER::Transition(mDepthStencilBuffer.Get(), D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_DEPTH_WRITE);
...
void Init::LogAdapterOutputs(IDXGIAdapter* adapter)
{
	UINT i = 0;
...
void Init::LogOutputDisplayModes(IDXGIOutput* output, DXGI_FORMAT format)
{
	UINT count = 0;
	UINT flags = 0;
